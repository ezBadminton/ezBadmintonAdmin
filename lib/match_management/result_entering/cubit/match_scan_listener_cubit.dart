...
/// the ID of a [TournamentMatch] model.
///
/// Whenever a complete sequence was parsed, the cubit looks up the ID and
/// emits the corresponding [TournamentMatch] object if it exists.
///
/// This is used to enable the functionality of the QR codes on the printed
/// game sheets.
/// The QR code contains a string of this format: `$match:<TournamentMatch ID>$`.
...
class MatchScanListenerCubit
  ...
  MatchScanListenerCubit({
    required ModelStore<TournamentMatch> matchDataRepository,
  })  : _inputBuffer = Queue(),
  ...
  void _onMatchIdParsed(String matchId) {
    ...
    TournamentMatch? match = state
        .getCollection<TournamentMatch>()
