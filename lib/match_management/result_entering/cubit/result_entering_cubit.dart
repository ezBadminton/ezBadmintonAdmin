...
class ResultEnteringCubit extends Cubit<ResultEnteringState> {
  ResultEnteringCubit({
    required this.match,
    ...
    required this.scoreEndpoint,
        ...
        super(const ResultEnteringState()) {
  ...
  final SetScoreEndpoint scoreEndpoint;
  ...
  void resultSubmitted() async {
    ...
    List<int> team1Points = setResults.map((set) => set.$1).toList();
    List<int> team2Points = setResults.map((set) => set.$2).toList();

    try {
      await scoreEndpoint.post(
        pathParams: {"matchdata": match.id},
        body: {"team1points": team1Points, "team2points": team2Points},
      );
  ...
  static List<ScoreInputController> _createScoreInputControllers(
    ...
    List<MatchSet> existingSets = match.sets;
